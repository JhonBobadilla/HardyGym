...
app.post('/register', (req, res) => {
    ...
    const sql = 'INSERT INTO datos (nombre, email, password, telefono, ciudad, subscription_start_date) VALUES (?, ?, ?, ?, ?, ?)';
    db.query(sql, [nombre, email, password, telefono, ciudad, subscriptionStartDate], (err, result) => {
        if (err) {
...
// Función falsa para simular la verificación de pago
function fakeVerifyPayment(reference, callback) {
    setTimeout(() => {
...
function authenticateToken(req, res, next) {
    ...
    if (!token) return res.redirect('/public/login.html');

    jwt.verify(token, secretKey, (err, user) => {
        if (err) return res.redirect('/public/login.html');
        req.user = user;
        next();
    });
}
