...
app.post('/login', (req, res) => {
    ...
    db.query(sql, [txtemail, txtpassword], (err, results) => {
        ...
        if (results.length > 0) {
            ...
            const user = { id: results[0].id, nombre: results[0].nombre, email: results[0].email };
            const token = jwt.sign(user, secretKey, { expiresIn: '1h' });
            res.json({ token, nombre: user.nombre }); // Enviar el nombre del usuario al cliente
        } else {
...
app.get('/getUserId', authenticateToken, (req, res) => {
    res.json({ userId: req.user.id, nombre: req.user.nombre }); // Incluye el nombre del usuario en la respuesta
});
